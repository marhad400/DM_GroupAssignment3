...
def main(): 
   # Cluster Evaluation: Intrinsic Sillhouette Score
    ...
    normalizedDf = kmeansHelper.normalizeWeather(df)
    k = 6
    ...
    for i in range(len(clusterPoints)):
      for j in range(len(clusterPoints[i])):
        ...
        # Separation point
        separationPoint = calculate_separation_point(clusterPoints[i][j], clustering.centroids, columns, i, clusterPoints)
    ...
    clusters, noise = clustering.dbscan(data_points, columns, eps, min_samples)
